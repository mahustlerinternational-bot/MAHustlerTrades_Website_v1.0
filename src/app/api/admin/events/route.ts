...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('events').insert({
    title:b.title, slug, description:b.description??null, event_type:b.event_type??'webinar',
    ...
    host_name:b.host_name??null, is_published:b.is_published??false, created_by:s.userId,
  }).select('*').single();
  if (error) return NextResponse.json({ error:error.message },{ status:500 });
