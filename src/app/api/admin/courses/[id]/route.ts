...
export async function GET(req: NextRequest,{ params }:{ params:{ id:string } }) {
  ...
  const { data, error } = await supabaseAdmin.from('courses').select('*').eq('id',params.id).single();
...
export async function PUT(req: NextRequest,{ params }:{ params:{ id:string } }) {
  ...
  const { data, error } = await supabaseAdmin.from('courses').update(u).eq('id',params.id).select('*').single();
  if (error) return NextResponse.json({ error:error.message },{ status:500 });
  return NextResponse.json(data);
}
export async function DELETE(req: NextRequest,{ params }:{ params:{ id:string } }) {
  const s = await requireAdminSession(req);
  if (!s) return NextResponse.json({ error:'Forbidden' },{ status:403 });
  const { count } = await supabaseAdmin.from('enrollments').select('*',{ count:'exact', head:true }).eq('course_id',params.id).eq('status','active');
