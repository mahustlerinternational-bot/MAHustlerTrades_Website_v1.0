...
export async function GET(req:NextRequest){
  ...
  if(!allowed)return NextResponse.json({error:'Premium or approved IB access is required'},{status:403});
  ...
  return NextResponse.json({data:signals,performance:signalPerformance(signals)});
}
