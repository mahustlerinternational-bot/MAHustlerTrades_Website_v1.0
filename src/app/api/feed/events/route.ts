...
export async function GET(req:NextRequest){
  ...
  if(profile.role!=='admin'&&profile.ib_status!=='active'&&!profile.package_id)return NextResponse.json({error:'Premium or approved Elite access is required'},{status:403});
  ...
  const {data,error}=await query;if(error)return NextResponse.json({error:error.message},{status:500});return NextResponse.json({data:data??[]},{headers:{'Cache-Control':'private, no-store, max-age=0'}});
}
