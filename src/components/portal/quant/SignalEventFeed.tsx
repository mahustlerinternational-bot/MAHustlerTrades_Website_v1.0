...
type Highlight=[string,string|number|null|undefined];
function signalHighlights(metrics:Record<string,unknown>):Highlight[]{
  if(!metrics.signal_type)return [];
  const low=Number(metrics.entry_zone_low),high=Number(metrics.entry_zone_high);
  const zone=Number.isFinite(low)&&Number.isFinite(high)
    ? `${Math.min(low,high)} – ${Math.max(low,high)}`
    : metrics.entry;
  return [
    ['Signal',metrics.signal_type==='long'?'BUY':'SELL'],
    ['Entry Zone',zone as string|number|undefined],
    ['TP1',metrics.tp1 as number|undefined],
    ['TP2',metrics.tp2 as number|undefined],
    ['TP3',metrics.tp3 as number|undefined],
    ['SL',metrics.stop_loss as number|undefined],
  ];
}
export default function SignalEventFeed({initialEvents}:{initialEvents:SignalFeedEvent[]}){
  ...
  return <section style={{background:'#111',border:'1px solid rgba(255,255,255,.07)',marginBottom:'2rem',animation:'fadeUp .5s .4s ease forwards',opacity:0}}>
    ...
    {events.length===0?<p style={{padding:'2.5rem',textAlign:'center',color:'#555',fontSize:'.75rem'}}>Telegram alerts, risk controls, regime updates, and performance reports will appear here.</p>:<div style={{maxHeight:'620px',overflowY:'auto'}}>{events.map(event=>{const color=COLORS[event.category]??'#888';const metrics=event.metrics??{};const highlights:Highlight[]=[...signalHighlights(metrics),['P&L',metrics.net_pnl as number|undefined],['Result',metrics.result_r!=null?`${Number(metrics.result_r)>0?'+':''}${metrics.result_r}R`:null],['Win Rate',metrics.win_rate_pct!=null?`${metrics.win_rate_pct}%`:null],['Drawdown',metrics.drawdown_pct!=null?`${metrics.drawdown_pct}%`:null],['Regime',(metrics.new_regime??metrics.regime??metrics.market_regime) as string|undefined]];const visibleHighlights=highlights.filter((x):x is [string,string|number]=>x[1]!=null);return <article key={event.id} style={{padding:'1rem 1.25rem',borderBottom:'1px solid rgba(255,255,255,.04)',borderLeft:`3px solid ${color}`}}>
      ...
      {visibleHighlights.length>0&&<div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginTop:'9px'}}>{visibleHighlights.map(([label,value])=><span key={label} style={{fontSize:'.58rem',padding:'3px 7px',background:`${color}11`,border:`1px solid ${color}33`,color}}>{label}: {String(value)}</span>)}</div>}
    </article>;})}</div>}
  </section>;
}
