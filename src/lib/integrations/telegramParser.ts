...
export interface ParsedTelegramEvent{
  ...
  normalized?:{
    ...
    exit?:number;result_r?:number;status?:'closed_tp'|'closed_sl'|'closed_manual';ticket?:string;regime?:string;notes?:string;
  };
}
...
function parseSection(body:string):ParsedTelegramEvent{
  ...
  if(/BUY TRADE OPENED|SELL TRADE OPENED|SIGNAL OPENED/.test(upper)){category='signal';severity='success';}
  ...
  if(category==='signal'&&inst&&side&&typeof metrics.entry==='number'&&typeof metrics.tp1==='number'&&typeof metrics.stop_loss==='number')normalized={action:'open_signal',instrument:inst,signal_type:side,entry:metrics.entry as number,tp1:metrics.tp1 as number,tp2:metrics.tp2 as number|undefined,tp3:metrics.tp3 as number|undefined,sl:metrics.stop_loss as number,notes:typeof metrics.setup_tags==='string'?metrics.setup_tags:undefined};
